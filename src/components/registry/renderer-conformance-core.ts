...
export type RendererTarget = "editor" | "preview" | "published";
...
/**
 * Evaluates supplied registrations. Supplied manifest-shaped data is checked
 * for drift but never becomes the source used for conformance.
 */
export function createRendererConformanceReportCore(
  input: RendererConformanceInput,
  bridgeComponentNames: readonly string[],
  ...
  const bridgedComponentTypes = new Set(bridgeComponentNames);

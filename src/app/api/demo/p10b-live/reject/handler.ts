...
function failure(status: number, category: "permissionDenied" | "stale" | "validation") {
...
export function createP10bLiveSynthesisRejectHandler({
  ...
  return async function POST(request: Request): Promise<Response> {
    ...
    try {
      const synchronization = await rejectP10bLiveSynthesisProposal({ ...body, environment });

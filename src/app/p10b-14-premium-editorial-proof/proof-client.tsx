...
function utilityRuntime(
  ...
  if (familyId === "cart")
    return {
      kind: "cart",
      revision: "p10b14-cart-r1",
      lines: [
        {
          lineId: "p10b14-cart-line",
  ...
  if (familyId === "error-state")
    return {
      kind: "error",
      revision: "p10b14-error-r1",
      message: { en: "We could not refresh this view.", fi: "Näkymää ei voitu päivittää." },
      recoverable: true,
      actions: ["retry"],
    };
  if (familyId === "not-found")
    return { kind: "not-found", revision: "p10b14-not-found-r1", actions: ["return-home"] };
  return undefined;
}

export function P10B14PremiumEditorialProofClient({
  aggregate,
  evidenceReferences,
  locale,
  page,
  ...
  if (page.pageFamily?.familyId === "product-detail") {
    ...
    if (!product || !presentation)
      throw new Error("The P10B-14 PDP proof requires current canonical product authority.");
    rendered = (
  ...
  return (
    <div
      data-p10b-14-complete-storefront="premium-editorial"

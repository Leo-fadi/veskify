...
} from "./whole-storefront-failures";
export {
  ServerWholeStorefrontAuthorityError,
  mapServerWholeStorefrontFailure,
} from "./whole-storefront-failures";
export type {
  ServerWholeStorefrontAuthorityErrorCode,
  ServerWholeStorefrontFailure,
} from "./whole-storefront-failures";
import { z } from "zod";
import {
...
function failure(error: unknown): Response {
  ...
  return response(mapped.status, {
    ok: false,
    failure: { category: mapped.category, retryable: mapped.retryable },
  });
}
...
export function createServerWholeStorefrontPlanningHandler({
  ...
  return async function POST(httpRequest: Request): Promise<Response> {
    ...
    try {
      ...
      diagnostic(stage, mapServerWholeStorefrontFailure(error).category, failed.status);
      return failed;
    }
  };
}

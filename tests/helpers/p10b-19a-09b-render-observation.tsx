...
export const P10B19A09B_BASE_COMMIT = "1ac8cab8a3bd15d2518e7e4ac0341fe64abaa935";
export const P10B19A09B_BASELINE_PATH =
  "/private/tmp/veskify-p10b-19a-09b-historical-v1-baseline.json";
...
export type P10B19A09BHistoricalRenderReplaySource = Readonly<{
...
function defaultRepresentativeSources(): readonly P10B19A09BHistoricalRenderReplaySource[] {
...
function resolvedSurface(
  ...
  const pathPrefix = previewPathPrefix(aggregate.project.id, "history", snapshot.id);
...
async function renderObservation(
  ...
  const resolved = resolvedSurface(surface, locale, aggregate, snapshot);
  ...
  const surfaceSelector =
    surface === "home"
      ? 'main#storefront-main-content > [data-component][data-variant][data-render-target="preview"]'
      : surface === "collection"
        ? '[data-component="dynamicCollectionCommerce"][data-render-target="preview"][data-search-context="none"]'
        : surface === "search"
          ? '[data-component="dynamicCollectionCommerce"][data-render-target="preview"][data-search-context="transient-canonical-results"]'
          : surface === "product-detail"
            ? '[data-component="dynamicProductDetail"][data-render-target="preview"][data-variant]'
            : surface === "content-support"
              ? '[data-component="contentSupport"][data-render-target="preview"][data-page-family][data-variant]'
...
export async function createP10B19A09BHistoricalRenderReplayEvidence(
  sources: readonly P10B19A09BHistoricalRenderReplaySource[] = defaultRepresentativeSources(),
) {
    ...
    for (const surface of surfaces) {
      for (const locale of locales) {
        renderObservations.push(
          await renderObservation(directionId, aliasId, surface, locale, aggregate, snapshot),
        );
      }
    }

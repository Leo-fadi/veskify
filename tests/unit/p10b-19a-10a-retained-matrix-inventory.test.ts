...
describe("P10B-19A-10A retained matrix inventory", () => {
  ...
  it("keeps all nine production authorities inactive and all governed adapters outside runtime", () => {
    ...
    const scopedConsumers = new Set([
      "src/components/storefront/composed-page-realization.ts",
      "src/data/demo/ar-06a-composed-template.ts",
    ]);
    const unexpectedConsumer = (file: string, source: string) =>
      !allowed.test(file) && !scopedConsumers.has(file) && marker.test(source);
    // prettier-ignore
    expect(sourceFiles(path.join(repositoryRoot, "src")).map((file) => path.relative(repositoryRoot, file).split(path.sep).join("/")).filter((file) => unexpectedConsumer(file, readFileSync(path.join(repositoryRoot, file), "utf8")))).toEqual([]);
    expect(
      unexpectedConsumer(
        "src/components/storefront/composed-page-realization.ts",
        "PageBlueprintV2",
    ...
    expect(
      unexpectedConsumer("src/data/demo/ar-06a-composed-template.ts", "page-blueprint-v2"),
    ).toBe(false);
    expect(
      unexpectedConsumer("src/components/storefront/unrelated-v2-renderer.tsx", "PageBlueprintV2"),
    ).toBe(true);
    expect(
      unexpectedConsumer(
        "src/components/storefront/composed-page-realization-copy.ts",
        "PageBlueprintV2",
      ),
    ).toBe(true);
    expect(
      unexpectedConsumer("src/data/demo/ar-06a-composed-template-copy.ts", "page-blueprint-v2"),
    ).toBe(true);
  });

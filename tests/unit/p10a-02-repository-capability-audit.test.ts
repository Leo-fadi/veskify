...
describe("P10A-02 repository capability audit", () => {
  it("keeps every legacy renderer registration represented in the canonical V2 registry", () => {
    expect(p10a02ComponentCapabilityAudit.v1RegisteredComponentTypes).toHaveLength(29);
  ...
  it("keeps every V2 component type connected to either the legacy bridge or an all-surface renderer", () => {
    expect(p10a02ComponentCapabilityAudit.v2RegisteredComponentTypes).toHaveLength(29);
    expect(p10a02ComponentCapabilityAudit.v2RegisteredVariantCount).toBe(126);

...
const presentationSource = readFileSync(
  resolve(process.cwd(), "src/components/onboarding/storefront-generation-review-presentation.ts"),
  "utf8",
);
...
describe("storefront generation review panel boundary", () => {
  it("consumes only the canonical review contract and stays presentation-only", () => {
    ...
    expect(presentationSource).toContain("@/application/storefront-generation-review/contract");
    expect(presentationSource).not.toMatch(
      /from ["']@\/application\/storefront-generation-review["']|from ["']@\/application\/storefront-generation-review\/projection["']|guided-storefront-generation|ProjectRepository|IndexedDB|@puckeditor|integrations\/puck/,
    );
  });
});

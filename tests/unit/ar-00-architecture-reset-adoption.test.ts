...
describe("post-pilot repository state and PR lifecycle", () => {
  it("accepts the intended candidate state without asserting that the PR has merged", () => {
    ...
    try {
      ...
      expect(tracker).toContain("effective upon explicit owner acceptance/merge of AR-05B");
...
describe("AR-05B bounded proposed status transition", () => {
  it.each(["AR-02", "AR-02M", "AR-23", "AR-30"])(
    "rejects stale or premature %s closure",
    (task) => {
      const directory = fixture();
      try {
        if (["AR-02", "AR-02M"].includes(task)) {
          const path = join(directory, "docs/VESKIFY_DEVELOPMENT_DELIVERY_TRACKER.md");
          const original = readFileSync(path, "utf8");
          const changed = original.replace(

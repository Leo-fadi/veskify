...
describe("post-pilot repository state and PR lifecycle", () => {
  it("accepts the intended candidate state without asserting that the PR has merged", () => {
    ...
    try {
      ...
      expect(tracker).toContain("effective upon explicit owner acceptance/merge of AR-02L");
...
describe("AR-00 architecture-reset adoption guard", () => {
  ...
  it.each([
    ...
    "AR-02K is the selected successor before AR-02J merges.",
    "AR-02K is the selected successor without safe synchronization.",
  ])("rejects an unauthorized successor in BATCH-08: %s", (successor) => {
    const directory = fixture();
...
describe("AR-02L bounded proposed status transition", () => {
  it.each(["AR-02", "AR-02L", "AR-23", "AR-30"])(
    ...
    (task) => {
      ...
      try {
        if (task === "AR-02L") {
          ...
          const original = readFileSync(path, "utf8");
          const changed = original.replace(
            `${task} is **Baseline / closed upon explicit owner acceptance/merge`,
            `${task} is **Baseline / closed`,
          );
          expect(changed).not.toBe(original);
          writeFileSync(path, changed);
          expect(() => check(directory)).toThrow(
            /conditional closure|contradictory current status/,
          );
          return;
        }

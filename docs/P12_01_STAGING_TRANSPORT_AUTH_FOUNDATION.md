...
P12-01 adds the shared server-only foundation that future Vesko staging adapters use. It does not
...
P12-01 module that reads environment configuration. Client components, domain contracts and
...
backend’s documented correlation-header name. A single deadline applies to authentication, fetch
and response parsing; cancellation wins even if an injected authentication or fetch dependency
ignores its signal. JSON parsing occurs only after successful status validation; invalid JSON maps
to a safe failure.

Authenticated requests always use `redirect: "manual"`. Every redirect is normalized as a staging
failure without inspecting or following its `Location`, so protocol-neutral credential headers are
never resent to another target. Routes are relative to and confined inside the configured base-path
prefix; dot segments (including encoded forms), backslashes, absolute URLs and namespace escapes
are rejected. `expectedStatuses` is restricted to 2xx successes, so authentication, conflict,
rate-limit, redirect and service failures cannot be made successful by an adapter override.
...
returned exactly as supplied: P12-01 does not parse, number-convert, compare or apply standalone
revision encoding to them.

The optional logger receives only operation name, method, an adapter-supplied validated route
template containing an explicit parameter (or a generic redacted label), status, duration,
correlation ID and typed failure category. It never derives a route from the raw request path and
never receives configuration values,
authorization headers, cookies, request/response bodies, merchant payloads or customer data.
Logger exceptions are swallowed so observability cannot change a request outcome. Unread response
bodies are cancelled for failures and `empty` responses; cancellation errors do not replace the
primary result.
...
the strict contract for that backend’s readiness response. P12-01 intentionally does not invent a
...
The implementation branch retains its historical `codex/p10-01-staging-transport-auth-foundation`
name, but this document is the canonical P12-01 deliverable. P10-01 remains reserved by the SDD
for the Storefront Studio product UX work.

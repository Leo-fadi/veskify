...
- **Authoritative specification:** `docs/VESKIFY_SDD.md`
- **Synchronized export:** `docs/VESKIFY_SDD_v1.1.docx`
- **Related decision:** ADR-001 — Puck as the embedded editor foundation
...
Merchant request
  ...
  -> proposal and preview without mutating the active draft
...
The merchant sees a concise explanation and visual proposal before accepting material changes. The active draft is not mutated until the merchant accepts the proposal. Rejected proposals leave the active draft unchanged. Saving a draft and publishing are separate actions: draft save persists unpublished work, while publishing requires a separate explicit confirmation.
...
- rejected or failed proposals leave the active draft intact;
...
1. Complete real Puck editing for approved components.
2. Expand the jewellery component vocabulary.
3. Define canonical operation schemas and executor.
4. Add proposal accept/reject/revise.
5. Implement the first 12–20 skills with deterministic fixtures.
6. Add chat, intent classification, and planning.
7. Add guided store generation.
8. Add real-data presentation enrichment.
9. Integrate save, preview, and explicit publish.
10. Connect the same contracts to Vesko services.

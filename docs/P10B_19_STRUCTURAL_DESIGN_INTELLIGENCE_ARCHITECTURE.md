...
```mermaid
flowchart LR
  A[Merchant request and approved references] --> B[AI Creative Brief]
  B --> C[Deterministic family candidate set]
  C --> D[Three distinct concept assignments]
  D --> E[Bounded Visual Recipe preferences]
  E --> F[Deterministic recipe and PageBlueprint compilation]
  F --> G[Exact registered component realization]
  G --> H[One materialization per immutable concept revision]
  H --> I[Rendered storefront]
  I --> J[Typed screenshot critic]
  J --> K[At most one bounded successor revision]
  K --> L[Human comparison and preference evidence]
  L --> M[Existing proposal lifecycle]
  M --> N[One StorefrontSnapshot lifecycle]
```
...
```mermaid
flowchart TD
  A[Studio merchant prompt] --> B[Canonical proposal route]
  B --> C[Server-owned refreshed project authority]
  C --> D[Prompted semantic provider]
  D --> E[Semantic intent v1 validation]
  E --> F[Capability projection and compatibility resolver]
  F --> G[One of three coordinated direction packages]
  G --> H[Exact compiled decision v2]
  H --> I[Coordinator and sole materializer]
  I --> J[Isolated proposal]
  J --> K[Accept or Reject]
  K --> L[Undo / Redo / Save / reload / Preview]
  L --> M[One StorefrontSnapshot and publication compiler]
```
...
```mermaid
flowchart TD
  A[Merchant prompt] --> B[Current facts, catalogue, approved assets, references]
  B --> C[Provider call 1: Creative Brief plus A/B/C hypotheses]
  C --> D[Strict brief validation and unsupported-intent ledger]
  D --> E[Deterministic compatible family enumeration]
  E --> F[Distinct family assignment A/B/C]
  F --> G[Provider call 2: bounded Visual Recipe preferences]
  G --> H[Deterministic recipe compiler]
  H --> I[PageBlueprint v2 selection and exact component realization]
  I --> J[Exact compiled concept decisions]
  J --> K[Existing sole materializer]
  K --> L[Three isolated immutable candidate revisions]
  L --> M[Machine gates and rendered captures]
  M --> N[Typed visual critic, maximum one pass]
  N --> O[Validated successor revision or no correction]
  O --> P[Human pairwise selection]
  P --> Q[Existing proposal and StorefrontSnapshot lifecycle]
```
...
```mermaid
flowchart LR
  A[StorefrontSnapshot page and BrandSystem] --> B[Veskify page-to-Puck projection]
  B --> C[Puck Config and Data in editor]
  C --> D[Validated bounded editor actions]
  D --> E[Veskify Puck-to-page validation]
  E --> F[Existing proposal/history/snapshot authority]
```
...
| -------- | ----------------------------------------------------------- | ------------------------------ | ------------------------------------------------------------------------------------------------------------- |
| P10B-19A | Structural Storefront Family and PageBlueprint v2 contracts | Accepted PRE                   | Versioned schemas, registry, selector, topology identity, and v1 compatibility; no new visual output required |
...
| 19A-02 | Versioned Structural Family registry and fingerprints in proposed `src/application/structural-storefront-families` | A-01              | Six identities are structurally non-empty and deterministic        | Mocks only; no screenshots                 | No renderers; merge registry                 |
...
| 19A-04 | Deterministic compatibility/selection and normalized topology v2                                                   | A-02/A-03         | Distinct family/topology selection with stable tie-breaks          | Mocks only                                 | No provider changes; merge selector          |
...
| 19G-11 | Content/support/utility closure                         | G-02 through G-10 | Full truth table, four widths, EN/FI, a11y, runtime transience                      | Deterministic; PO comparison | No live AI; merge 19G closure                              |

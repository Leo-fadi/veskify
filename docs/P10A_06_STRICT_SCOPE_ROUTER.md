...
The request declares one canonical P10A-06 scope: `selectedSection`, `currentPage`, `sharedStorefrontFrame`, `designSystem`, or `completeStorefront`. The declaration must exactly match the canonical package and its page and slot authority:

- `applyExactBrandPalette` is design-system-only and cannot acquire page or slot authority.
- `improveHero` is one exact current home or landing hero slot within `selectedSection`, with an explicit executable profile.
- `addCampaignSection` is one explicitly authorized `currentPage` insertion; its optional profile remains resolved by P10A-05D-02 from current materialization.
...
`sharedStorefrontFrame` is canonical but has no P10A-05B executable package yet. A request for navigation, header, announcement bar, or footer receives typed `clarificationRequired` / `unsupportedCanonicalScope` rather than a malformed-request result or an automatic widening to complete storefront. The same applies to non-package selected-section and current-page requests.
...
The router also requires the resolved canonical package to agree with the nested follow-up authority package after the same registry resolution. A request cannot report a narrow package while dispatching a broader governed package.
...
Before a route-only follow-up decision succeeds, the router calls P10A-05D-02's route-authority validator. It checks the current planning input, page identity/type, executable profile materialization, and exact slot ownership without compiling a proposal or creating lifecycle side effects.

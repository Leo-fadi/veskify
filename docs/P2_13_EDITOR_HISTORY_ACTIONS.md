...
- `CanonicalEditorHistory` initializes and validates per-page histories, commits atomic page values,
  undoes, redoes, resets to the saved baseline and rebases after a successful draft save.
...
History retains at most 50 canonical states per page by default. Inputs, stored states and returned
states are cloned, and every commit passes the supplied registered-page validator. A divergent
commit clears redo history.

## Editor behaviour

- Every valid manual Puck handoff is one history entry.
...
- Duplicate and hide/show use the actual selected Puck section. Header and footer remain visible,
  single-instance sections. Hidden sections keep an editor-only selectable placeholder.
...
- Discard resets the current page to its latest saved baseline and clears that page's undo/redo
  history. It also invalidates an open proposal based on discarded edits.

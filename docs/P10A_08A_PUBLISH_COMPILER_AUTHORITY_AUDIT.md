...
**P10A-08A exclusion:** provider calls, storefront publication, runtime/schema changes,
golden-baseline changes, and changes to the authoritative SDD or synchronized DOCX.
...
P10A-08A is documentation/audit-only. It therefore leaves `docs/VESKIFY_SDD.md` and the
synchronized DOCX unchanged. Later P10A-08 implementation work must update both in the same
implementation task, or in the final Phase 10A closure task, whenever it materially changes a
publishing, compiler, acceptance-lineage, gateway, persistence, rollback, or rendering contract.
No material architecture or contract change may land without that authoritative documentation
synchronization.
...
This documentation synchronization rule applies to later implementation tasks as well as the
technical requirements above; P10A-08A's own SDD/DOCX exclusion is not an exception for them.
...
AI/provider proposal or manual editor change
  ...
  -> published homepage resolves project.publishedSnapshotId
     and passes renderTarget="published"
  -> published collection/PDP wrappers resolve project.publishedSnapshotId,
     but their nested render contexts currently default to "preview"
...
| Active rendering      | `preview-mode.ts`; `project-preview-client.tsx`; `collection-preview-client.tsx`; `product-preview-client.tsx`; published wrappers                                 | `snapshotKind="published"` resolves `publishedSnapshotId` for homepage, collection and PDP. Homepage `ProjectPreviewClient` explicitly passes `renderTarget: "published"`; collection/PDP nested contexts omit it and therefore use `createStorefrontRenderContext`'s default `"preview"`. Commerce-route `target` props do not change that context default. | Snapshot selection is correct. Collection/PDP have a nested render-context adapter gap; immutable compiled runtime remains a separate gap. |
...
| Is output exact published snapshot?                                   | Homepage, collection and PDP select `publishedSnapshotId`; homepage's render context explicitly uses `"published"`. Collection/PDP contexts still default to `"preview"`, so published snapshot selection does not by itself prove published render-target semantics. No persisted compiled-runtime fingerprint/parity assertion binds every route to compiler output. |
| Are preview and published separate?                                   | Snapshot selection/path prefixes are separate. Homepage render target is separate too. Collection/PDP wrappers label and select published state, but their nested render context defaults to `"preview"`; this is a narrow adapter gap, not a failure to select the published snapshot.                                                                                |
...
P10A-08B owns closure of the merchant-route bypass: it must move or strictly gate execution in
`src/app/projects/[projectId]/publish/publish-client.tsx` through the authoritative application
publishing gateway. The route must preserve authenticated identity and `publishStorefront`
permission checks, bind the request to the accepted-snapshot receipt, retain stale-state and
durable-idempotency controls, and leave separately authorized development/test repository adapters
available only behind explicit standalone boundaries.
...
| Item               | Plan                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| ------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Objective          | Bind reviewed accepted canonical `StorefrontSnapshot` to immutable server-owned acceptance receipt, and route or strictly gate merchant publication through the authoritative gateway. Require exact receipt/snapshot/project/fingerprint lineage for P10A publishing while preserving separately authorized manual-save semantics.                                                                                                                                      |
| Owning layer       | Application/domain acceptance and publishing contracts; authoritative integration boundary.                                                                                                                                                                                                                                                                                                                                                                              |
| Likely files       | `src/application/controlled-acceptance-preflight/**`, `whole-storefront-proposal-lifecycle/**`, `src/application/publishing/{contract,prepare-publish,confirm-publish}.ts`, `src/application/vesko-integration/contract.ts`, `src/integrations/vesko-publishing/authoritative-publishing-adapter.ts`, `src/app/projects/[projectId]/publish/publish-client.tsx`, `src/app/projects/[projectId]/publish/page.tsx`, focused helpers/tests.                                 |
...
| Tests required     | accept → receipt → save/reload → publish; pending/rejected/stale/foreign/mismatched receipt rejection; newer save stales receipt; merchant route invokes the authoritative gateway; direct browser repository publication is unavailable or explicitly gated; permission failure occurs before persistence; duplicate submission remains idempotent; stale accepted lineage cannot publish; declared manual-save policy remains separate; provider cannot forge receipt. |
...
| Documentation      | Update the authoritative SDD and synchronized DOCX in this task if receipt, gateway, acceptance-lineage, or merchant publish-route contracts materially change; otherwise carry the exact update into the final Phase 10A closure task.                                                                                                                                                                                                                                  |
| Out of scope       | UI redesign beyond the authority migration, provider call, PageBlueprint redesign, commerce mutation, one-click public rollback.                                                                                                                                                                                                                                                                                                                                         |
...
| Documentation      | Update the authoritative SDD and synchronized DOCX in this task if compiler, persistence, publication, or rollback contracts materially change; otherwise carry the exact update into the final Phase 10A closure task.                                                                                                |
...
| Item               | Plan                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| ------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Objective          | Propagate `renderTarget: "published"` through published collection and PDP nested render contexts, preserve `"preview"` on preview contexts, and prove accept → compile → publish → exact homepage/collection/PDP render → restore-to-draft → explicit republish without provider call at publish.                                                                                                                                                                                  |
| Owning layer       | Integration/e2e evidence and renderer route adapters; no new authority.                                                                                                                                                                                                                                                                                                                                                                                                             |
| Likely files       | `src/app/projects/[projectId]/collections/[collectionSlug]/collection-preview-client.tsx`, `src/app/projects/[projectId]/products/[productSlug]/product-preview-client.tsx`, published collection/PDP wrappers, focused specs near `tests/integration/publish-route.test.tsx`, `tests/integration/collection-route.test.tsx`, `tests/integration/product-route.test.tsx`, `tests/e2e/publish-confirmation.spec.ts`, `tests/e2e/history-restore.spec.ts`, and P10A evidence helpers. |
...
| Tests required     | Exact compiled/published snapshot identity and render target together; homepage, collection and PDP all use published rendering semantics; preview routes remain `"preview"`; no nested client silently defaults to preview when `snapshotKind="published"`; draft/published isolation; stale receipt; no provider invocation; asset/media/price/stock preservation; 375/768/1024/1440 browser evidence.                                                                            |
...
| Documentation      | Update the authoritative SDD and synchronized DOCX in this task if published render-target or rendering-evidence contracts materially change; otherwise carry the exact update into the final Phase 10A closure task.                                                                                                                                                                                                                                                               |
...
2. The merchant publish route invokes the authoritative gateway; direct browser-repository publication is unavailable or explicitly gated, and permission failures occur before persistence.
3. Publish accepts no proposal body, Puck state, provider payload, browser runtime tree or arbitrary saved snapshot; it consumes authoritative accepted lineage. Stale accepted lineage cannot publish, duplicate submissions remain idempotent, and manual-save semantics remain separate from reviewed AI proposal publication.
4. Compiler output/fingerprint is deterministic and publish makes no provider/LLM/Puck/browser-credential call.
5. Every P10A failure leaves draft, published pointer, history, artifact and idempotency record unchanged; no partial runtime becomes active.
6. Compile/runtime identity is atomically retained with published/synchronized snapshots; stale and ambiguous retries preserve current guarantees.
7. Homepage, collection and PDP routes select the exact committed published snapshot **and** use published render-target semantics; preview routes remain preview and no nested client silently defaults to preview.
8. Canonical IDs, price, compare-at, SKU, options, availability, routes and product media remain canonical; assets preserve provenance.
9. Restore is a new draft; explicit republish creates a new immutable publication; keyboard/focus and 375/768/1024/1440 proof pass.

## 13. Recommended PR decomposition

1. **08B:** receipt/lineage contract, authoritative merchant-route migration/gating, and tests; merge after W1 P10A-07C work is stable and without overlap in W1 canonical ownership. Synchronize SDD/DOCX for material gateway or acceptance-lineage changes.
2. **08C:** compiler receipt plus existing repository/gateway transaction integration; depends on 08B and P10A-03/04. Synchronize SDD/DOCX for material compiler, persistence, publication, or rollback changes.
3. **08D:** published collection/PDP render-target propagation plus renderer/e2e/manual evidence; consumes 08B/08C and does not redefine persistence/publish authority. Synchronize SDD/DOCX if the rendering contract changes materially and it was not already synchronized by an owning task.
...
- `tests/e2e/publish-confirmation.spec.ts` and `tests/e2e/history-restore.spec.ts`: local IndexedDB publish, immutable published snapshot navigation, restore-to-draft and confirmation viewport checks. They do not prove collection/PDP nested render-target propagation.

Manual/browser QA after implementation: homepage, collection and PDP for complex product and non-jewellery catalogue at 375/768/1024/1440; visible published-vs-draft isolation after later edit; explicit inspection of published versus preview render target for all three routes; stale acceptance/publish and compiler-failure messages; keyboard-only accept/review/publish/restore; and canonical price, availability, options, routes and media correctness.
...
- No P10A-03/04/07 contract, golden-baseline, or W1-owned change as part of P10A-08A. This audit leaves SDD/DOCX unchanged; later material P10A-08 contract work must synchronize them as stated above.

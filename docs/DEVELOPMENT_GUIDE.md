...
> **AR-05A compiled-composition candidate projection:** This repository state takes effect upon accepted
> merge of AR-05A; it does not assert the PR's lifecycle or owner acceptance.
> AR-00 is Baseline / closed. AR-01 is Baseline / closed. AR-02A is Baseline / closed. AR-02B is Baseline / closed. AR-02C is Baseline / closed. AR-02D is Baseline / closed. AR-02E is Baseline / closed. AR-02F is Baseline / closed. AR-02G is Baseline / closed. AR-02H is Baseline / closed. AR-02I is Baseline / closed. AR-02J is Baseline / closed. AR-02K is Baseline / closed. AR-02L is Baseline / closed. AR-02M is Baseline / closed.
> AR-02 is Baseline / closed; complete original acceptance is mapped in AR_02_BOUNDARY_ACCEPTANCE.md. AR-04 is Baseline / closed. AR-05 is Partial; canonical nesting/version dispatch remains AR-05B work. AR-05A is Baseline / closed effective upon accepted merge. AR-06 remains dependent on AR-05; positive collection/search and PDP composition must be proven by its executable commerce support before enablement. AR-03 is Baseline / closed.
...
P10B-18C, P10B-16P-05B and P10B-19 PRE are **Baseline**. P10B-18D is a **Baseline diagnostic
with live commercial quality rejected**. P10B-18 and P10B remain **Partial**. DEVX-01A through
...
identity is fabricated. A-09B owns historical snapshot read/render replay; A-09C owns the
now-complete publication replay boundary and A-09 closure. A-09A therefore claims no historical
classification, read/render replay,
publication compatibility, current-generation change or merchant-visible outcome.

P10B-19A-09B owns strict read-only historical-v1 snapshot validation and current-renderer replay.
It validates the canonical snapshot and current registered component, PageBlueprint-profile,
shared-frame, navigation, dynamic-commerce and catalogue authorities without write-back. An
optional A-09A replay reference is either exact-bound or rejected; without it, the disposition is
`readable-unattributed`, never inferred lineage. The deterministic deeply immutable receipt reports
seven persisted selection fields as verified and seven absent fields as unverified, reports
normalization disposition `none` or `canonical-read-defaults`, and uses
`legacy-v1-historical-snapshot-read-v1_<canonical-length>_<sha256>`. Frozen exact-base evidence
matches all 36 of 36 bounded current-renderer observations across the three representatives in
EN/FI with zero snapshot mutation, repository write, commerce change, media change, or
route/profile/frame mismatch. This proves neither raw HTML equality nor original lineage,
migration, publication replay, v2 authority, or merchant-visible change.

P10B-19A-09C owns bounded historical-v1 publication replay. It consumes the exact validated A-09B
result and receipt, makes a detached transient projection in which that historical snapshot is the
draft, and invokes the current canonical publication compiler with truthful `manual` source
authority. It requires current migration status and never writes a migration or replacement
snapshot. A strict immutable replay receipt binds the A-09B read authority, source snapshot and
current compiler fingerprints; parsing and current-authority assertion fail closed on stale or
contradictory evidence. Frozen exact-base proof matches three direct compilations, three
preparations and exactly three isolated atomic confirmations, including the active compiled
artifact and added publication version. All 36 of 36 published renderer observations and all 36
of 36 normalized Preview/published structural observations match, while provider, Vesko and
external-publication calls remain zero. Source aggregates and snapshots are unchanged; no
production/runtime consumer, v2 activation or merchant-visible change is added. Parent
P10B-19A-09 is Baseline / closed.

P10B-19A-10 is delivered through three bounded children without changing the accepted architecture:
A-10A owns the retained inventory and frozen baseline; A-10B owns cross-authority integration and
negative-failure coverage; A-10C owns complete retained execution, the closure report and the
product-owner architecture gate. A-10A freezes 24 explicit matrix entries, 76 exact task-base
production-source hashes, nine zero-count v2 inactivity assertions, exact 126-case/frozen-72 and
A-09 metrics, and dynamic DEVX-01 CI discoverability in a fresh-clone-safe checked fixture. It adds
zero production files and no runtime consumer.
...
A-10B2 binds exactly 36 ordered negative cases across the existing A-03 through A-09C owner
boundaries. Each case starts from fresh A-10B1 authority, applies one atomic corruption, records
the exact terminal phase and bounded typed/native-error or sorted Zod issue authority, completes
zero downstream phases, produces zero partial outputs and preserves its source fingerprint. The
matrix records zero repair, fallback, default, repository-write, provider, Vesko and external
publication activity. Cases 17, 18 and 20 alone permit native `Error` with null `errorCode`; case
25 binds contradictory substitution authority to `stale-selection-authority`; case 28 treats the
coherent production-empty root replacement and deterministic dependent recomputation as one
atomic corruption. Four legacy/v2 substitutions are rejected without coercion.

The strict fresh-clone-safe failure baseline has fingerprint
`p10b-19a-fail-closed-cross-authority-matrix-v1_30308_ffae8e9700b84c08fcf1fe7315846077b01170f0be0d82743dce4e8a0ac8d307`
and checked-fixture SHA-256
`8708a2282749796c9d27d2dcd382384989c7738a5eb0b2f72f87c1ad73cb6a64`. Its read-only
...
The product owner explicitly approved the original ten-parent delivery sequence in the immutable
DEVX-01A contract; the later A-08, A-09 and A-10 subchildren are bounded delivery decompositions of
three of those parents. This supersedes only P10B-19 PRE's earlier six-child delivery partition and
does not change Structural Storefront Family, PageBlueprint v2, registry, compatibility, migration
or closure ownership in the accepted architecture.

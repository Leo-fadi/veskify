...
The authority has its own stable fingerprint. Record creation rejects mismatched current authority.
...
A required failure yields `failed`; a required block yields `blocked`; only a complete all-passed record yields the protocol-only `passed` disposition.
